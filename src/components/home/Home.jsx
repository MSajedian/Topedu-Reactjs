...
function Home(props) {
    ...
    const getCoursesOfInstitution = () => {
        if (institutions[0]) {
            try {
                fetch(`${BackendURL}/institutions/courses/${institutions[0]._id}`, {
                    credentials: 'include',
                    method: 'GET',
                    headers: { 'Content-Type': 'application/json' },
                })
            ...
            <div className="UserOverview pt-2">
                <Container>
                    <div className="d-flex">
                        ...
                        <div className="p-2 h1">{userType === "owner" || userType === "instructor" ? <Button variant="primary" onClick={handleShowCreateCourseModal}>+&nbsp;Create&nbsp;Course</Button> : <></>}</div>
                    </div>
                                            ...
                                            <Col key={`course${index}`}>
                                                <Card className="position-relative border border-secondary" >
                                                    ...
                                                    <Card.Body className="border border-primary border-end-0 border-bottom-0 border-start-0">
                                                        ...
                                                        {userType === "owner" || userType === "instructor" ?
                                                            <Button id={`delete-button${index}`} className="btn btn-danger position-absolute bottom-0 start-0 m-1" onClick={() => (deleteCourse(course._id))}>
